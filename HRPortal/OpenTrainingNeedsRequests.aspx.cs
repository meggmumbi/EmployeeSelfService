...
namespace HRPortal
{
    public partial class OpenTrainingNeedsRequests : System.Web.UI.Page
    {
        ...
        protected void cancelapproval_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                String status = Config.ObjNav.CancelTrainingNeedsApproval(Convert.ToString(Session["employeeNo"]),IndividualPCNo);
        ...
        protected void sendapproval_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                String status = Config.ObjNav.SendTrainingNeedsApproval(Convert.ToString(Session["employeeNo"]), IndividualPCNo);
                String[] info = status.Split('*');
                if (info[0] == "success")
                {
                    documentsfeedback.InnerHtml = "<div class='alert alert-success'>" + info[1] + " <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a></div>";
