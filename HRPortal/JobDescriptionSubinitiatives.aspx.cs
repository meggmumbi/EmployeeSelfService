...
namespace HRPortal
{
    public partial class JobDescriptionSubinitiatives : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                var tunitofmeasure = nav.UnitsOfMeasure;
                ...
                var ttxtunitofmeasures = nav.UnitsOfMeasure;
                txtunitofmeasures.DataSource = ttxtunitofmeasures;
                txtunitofmeasures.DataValueField = "Code";
                txtunitofmeasures.DataTextField = "Description";
                txtunitofmeasures.DataBind();
        ...
        protected void apply_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var ttxtsubindicator = subindicator.Text.Trim();
                String status = Config.ObjNav.FnNewIndividualCardJobDescriptionSubActivities(ScoreCardId, ActivityNo, tsubinitiative, tpercentage, tactivitytargets, tstartdates, tduedates, tunitofmeasure, ttxtsubindicator);
        ...
        protected void Update_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string ttxtunitofmeasures = txtunitofmeasures.SelectedValue;
                ...
                var dtxtsubindicator = txtsubindicator.Text.Trim();
                String status = Config.ObjNav.FnUpdateIndividualCardJobDescirptionSubActivities(ScoreCardId, ActivityNo, tsubinitiative, tpercentage, tactivitytargets, tstartdates, tduedates, tentryNumber, ttxtunitofmeasures, dtxtsubindicator);
                String[] info = status.Split('*');
                if (info[0] == "success")
                {
                    generalFeedback.InnerHtml = "<div class='alert alert-success'>" + info[1] + " <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a></div>";
        ...
        protected void deleteSubActity_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                String status = Config.ObjNav.FnDeleteIndividualCardJDDescriptionSubActivities((String)Session["employeeNo"], ScoreCardId, ActivityNo, tsubinitiativeEntry);
                String[] info = status.Split('*');
                if (info[0] == "success")
                {
