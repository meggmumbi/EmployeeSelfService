...
namespace HRPortal
{
    public partial class OpenRecordRequisitions : System.Web.UI.Page
    {
        ...
        protected void sendApproval_Click(object sender, EventArgs e)
        {
            try
            {
                String trequestNumber = recordToApprove.Text.Trim(); ;
        ...
        protected void cancelApproval_Click(object sender, EventArgs e)
        {
            try
            {
                String tDocumentNo = approverecordoNo1s.Text.Trim();
                String status = Config.ObjNav.CancelFileRecordApproval(tDocumentNo);
                String[] info = status.Split('*');
                if (info[0] == "success")
                {                
                feedback.InnerHtml = "<div class='alert alert-success'>" + info[1] + "<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a></div>";
