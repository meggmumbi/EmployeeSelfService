...
namespace HRPortal
{
    public partial class AdditionaActivitiesSubinitiatives : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                var ttxtunitofmeasure = nav.UnitsOfMeasure;
                txtunitofmeasure.DataSource = ttxtunitofmeasure;
                txtunitofmeasure.DataValueField = "Code";
                txtunitofmeasure.DataTextField = "Description";
                txtunitofmeasure.DataBind();
            }
        }
        protected void apply_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var units = unitofmeasure.SelectedValue;
                var tsubindicator = subindicator.Text.Trim();
                String status = Config.ObjNav.FnNewIndividualCardSubActivities(ScoreCardId, ActivityNo, tsubinitiative, tpercentage, tactivitytargets, tstartdates, tduedates, units, tsubindicator);
        ...
        protected void Update_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string ttxtunitofmeasure = txtunitofmeasure.SelectedValue;
                ...
                var tsubindicator = txtsubindicator.Text.Trim();
                String status = Config.ObjNav.FnUpdateIndividualCardSubActivities(ScoreCardId, ActivityNo, tsubinitiative, tpercentage, tactivitytargets, tstartdates, tduedates, tentryNumber, ttxtunitofmeasure, tsubindicator);
                String[] info = status.Split('*');
                if (info[0] == "success")
                {
                    generalFeedback.InnerHtml = "<div class='alert alert-success'>" + info[1] + " <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a></div>";
